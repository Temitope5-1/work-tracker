...
for day in range(1, num_days + 1):
    ...
    if has_entry:
        ...
        hours_display = entry['hours']
        if hours_display == 0:
            day_label += f" 🏠 (Day off - 0 hrs)"
        else:
            day_label += f" ✅ ({hours_display} hrs)"
    else:
    ...
    with st.expander(day_label, expanded=is_today and not has_entry):
        ...
        with col1:
            hours = st.number_input(
                ...
                help="Enter 0 for days off, sick days, or holidays"
            )
        ...
        with col2:
            notes = st.text_input(
                ...
                placeholder="e.g., Day off, Sick, Holiday"
            )
        ...
        with col_btn1:
            if st.button("💾 Save", key=f"save_{date_key}", use_container_width=True):
                # Allow saving even with 0 hours
                ...
                save_data()
                if hours == 0:
                    st.success(f"Logged day off (0 hours)")
                else:
